...
JNIEXPORT jint JNICALL Java_com_limo_emumod_bridge_NativeDisplay_bufSize(JNIEnv *, jclass, const jlong audio) {
    const auto nativeDisplay = reinterpret_cast<NativeDisplay*>(audio);
    std::lock_guard lock(nativeDisplay->mutex);
    return static_cast<jint>(nativeDisplay->bufSize);
}

JNIEXPORT void JNICALL Java_com_limo_emumod_bridge_NativeDisplay_update(JNIEnv *env, const jobject obj, const jlong audio) {
    const auto nativeDisplay = reinterpret_cast<NativeDisplay*>(audio);
    std::lock_guard lock(nativeDisplay->mutex);
