...
Int8ArrayPacket::Int8ArrayPacket(const PacketType type, const jUUID* ref, const std::vector<uint8_t> &data)
    : type(type), ref(ref), data(data) { }

std::unique_ptr<Int8ArrayPacket> Int8ArrayPacket::unpack(const ENetPacket *packet) {
    const auto type = static_cast<PacketType>(packet->data[0]);
...
ENetPacket* Int8ArrayPacket::pack() const{
    ...
    memcpy(&packed[25], data.data(), data.size());
